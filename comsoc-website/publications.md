...
<b>Description:</b> {{ curated_list.description }}<br>
List maintained by {% for maintainer in curated_list.maintainers %}{% if maintainer.url %}<a href="{{ maintainer.url }}">{% endif %}{{ maintainer.name }}{% if maintainer.url %}</a>{% endif %}{% unless forloop.last %}{% if forloop.rindex == 2 %} and {% else %}, {% endif %}{% endunless %}{% endfor %}.
</li>
{% endfor %}
</ul>
